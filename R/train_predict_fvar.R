...
train_predict_fvar <- function( df, settings, soilm_threshold, weights=NA, plot=FALSE, verbose = FALSE ){
  ...
  ##------------------------------------------------
  ## If multiple layers of soil moisture data are available, do subset w.r.t. soil layer with highest value
...
train_predict_fvar_byrep <- function( irep, df, idxs_moist, settings, weights=NA, plot=FALSE, verbose, ... ){
  ...
  if ( length(idxs_moist)>30 && (nrow(df) - length(idxs_moist))>30 ){
    ...
    if (settings$package == "nnet") {
      ...
      # ### USE NNact MODEL WITH SM SET TO 1 TO PREDICT PET (alternative to current implementation -- not shown in the paper)
    ...
    df_cv_act <- out_nn_act$df_cv %>%
      rename(obs := !!settings$target) %>%  # rename "obs" to "ET" (the exclamation marks are there to read "ET" from the value stored in settings)
      left_join(df %>%
