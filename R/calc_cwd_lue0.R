...
calc_cwd_lue0 <- function(df, inst, nam_lue, do_plot = FALSE, verbose = FALSE){  
  ...
  # initialize variables (altrimenti mi da errore in "return" se una di queste non esiste per un site)
  cp1 = NA
  cp2 = NA
  slope1 = NA
  slope2 = NA
  ...
  if (nlue > 10){
    ...
    df <- df %>%
      ...
      mutate(bin = ntile(deficit, nbin)) # same number of elements within each bin
    # mutate(cwd_lower = as.numeric( sub("\\((.+),.*", "\\1", bin)),# only needed for CWD midpoint
    #        cwd_upper = as.numeric( sub("[^,]*,([^]]*)\\]", "\\1", bin) )
    # ) %>%
    #mutate(cwd_mid = (cwd_lower + cwd_upper)/2) %>%  # calculate median instead in every bin
    
    #  use the median inside each bin as mid point 
    df_grouped = df %>% 
      group_by(bin) %>% 
      summarise(
        cwd_mid = median(deficit, na.rm = TRUE),
      ) %>% 
      ungroup()
    
    df = df %>% 
      left_join(df_grouped, by = "bin") %>% 
      dplyr::select(-bin)
    
    df_agg <- df %>%
      ungroup() %>%
      group_by(cwd_mid) %>%
      summarise(lue = quantile(lue, probs = 0.5, na.rm = TRUE)) %>% # do the median for this application
      #summarise(lue = quantile(lue, probs = 0.9, na.rm = TRUE)) %>%  # summarise in lue upper quantile: that's why all the points are in the upper end
      as.data.frame()
  ...
  # filtrare siti con breaking points erronei (senza un vero drop in fET)
    if (min(df_agg[,2], na.rm = TRUE) > 0.7) { # se il bin con fET minore è cmq più alto di 0.7
      npsi = 0                  # considera direttamente tipo 0 
      type = 0
    }
