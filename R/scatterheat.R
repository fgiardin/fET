scatterheat <- function(df, X, Y, title, path){
  ...
  # expression(paste(ET[NN], "(mm d"^"-1"*")"))
  # expression(paste(PET[NN]))
  # expression(paste(ET[obs]))
  ...
  # density plot
  png(filename = path, width = 4, height = 4.2, units = 'in', res = 300) # mettere result_path in funzione di argomento path e test.png in funzione di title
