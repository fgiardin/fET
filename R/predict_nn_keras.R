predict_nn_keras <- function(df,
  ...
  if (identical(NULL, nn)){ # if we don't provide a previous model through nn argument
    ...
    #we do NOT shuffle for this application (otherwise: auto-correlation; values are not temporally independent)
    df_shffld <- df
...
predict_nn_keras_byfold <- function(df,
  ...
  model <- keras_model_sequential() %>%
    ...
    #layer_batch_normalization() %>% # normalize after every transformation performed by the network (can improve performance)
  ...
  if (length(idx) > 0){
    df_test <- df_test %>% dplyr::select(one_of(predictors))
    df_test <- predict(pp_stats, df_test)    # scale test data USING STATS FROM TRAINING
