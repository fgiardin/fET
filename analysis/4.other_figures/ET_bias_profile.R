### print ET bias profile (current figure 3)
library(grid)
library(tidyverse)
library(ggpubr)
...
load("./output/BE-Vie/data_frames/out_BE-Vie.RData") # load output 
load("./output/BE-Vie/data_frames/ddf_BE-Vie.RData") # load input 
...
a <- df_test1 %>%
  ...
  annotation_custom(grob_a) + # add name of site label
...
load("./output/IT-CA3_SWC_F_MDS_4/data_frames/out_IT-CA3.RData") # load output
load("./output/IT-CA3_SWC_F_MDS_4/data_frames/ddf_IT-CA3.RData") # load input 
...
# create df_test across all sites
ddf_out <- do.call( # come una loop ma scritta in forma compatta
    ...
    function(file){ # file è l'elemento i della lista (percorro la lista riga per riga)
      ...
      out$df_all <- out$df_all %>%  # df_cv doesn't change scatter plots (ma visto che non ha fvar, i density plot non funzionano)
        ...
        mutate(bias_act = nn_act - obs,
               ...
               bias_nn_norm = bias_nn/as.numeric(quantile(nn_act, 0.75, na.rm = TRUE)), # as.numeric serve a prendere solo il risultato e non il "75%"
...
# get raw soil moisture (not normalized between 0 and 1)
...
ddf_allsites <- do.call( # come una loop ma scritta in forma compatta
  ...
  lapply( # apply function to a list of objects 
    ...
    function(file){ # file è l'elemento i della lista (percorro la lista riga per riga)
      ...
      if(dim(ddf)[1] == 0){} # se il df non esiste, non fare niente
      else{
      ddf <- ddf %>% 
        mutate(name_site = substr(basename(file), 5, 10)) %>% 
...
c <- ddf_merge %>% 
  ...
  #dplyr::filter(SM_maxnorm_bin == "(-0.001,0.1]") %>% # retain only lowest SM bin ??? HOW TO DO IF NOT NORMALIZED BTW 0 AND 1 ???
