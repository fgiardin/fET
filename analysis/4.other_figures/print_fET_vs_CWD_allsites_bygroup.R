...
for (i in 1:length(medium_fET)){
  ...
  a <- b + labs(y = "fET (-)",
                ...
                title = site) +
    theme_classic() +
    #geom_point(data = df_EVI, aes(x = deficit, y = EVI_scaled/coeff), size=1.5, alpha = 0.5, color="#0a6228", position = position_stack(reverse = TRUE)) + # Divide by coeff to get the same range than other variable
    ...
    # scale_x_continuous(breaks = seq(0, 300, 100), limits = c(0, 325), expand = c(0, 0))
  c <- ggplot() + geom_line(data = df_EVI, aes(x = medianCWD, y = perc_50), color="#0CFA07", alpha=0.95, size = 1)
  c
