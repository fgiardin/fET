...
dir_name = sprintf("./output/%s", sitename) # path to directory of site (gia creata in pre_process)
...
settings <- list( 
  ...
  #predictors = c("TA_F", "VPD_F", "PPFD"), # predictors for LUE
  # predictors = c("NETRAD","VPD_F", "TA_F", "EVI"), # predictors for nn_act
  varnams_soilm  = "SWC_F_MDS_5", # "SWC_F_MDS_1", "wscal", "wcont_splash"
  rowid          = "date",
  threshold      = 0.5,
  package        = "keras", # "nnet" or "keras"
  print_model_summary = TRUE, 
  nrep          = 5,
  nfolds        = 5, # number of cross-validation folds
  
  #number of nodes per layer, can specify different number across layers
  #for nnet: relevant only first element of the vectors (only one layer)
  num_units_per_layer = c(8, 32, 64, 32, 16), 
  
  ## KERAS specific settings (harmless but useless with other packages)
  batch_size          = 64, 
  num_epochs          = 30, 
  val_size            = 0.25,
  learning_rate       = 0.01, # optimized value 
  num_layers          = 2 # number of hidden layers (only for keras)
)

# CHANGE SOIL VARIABLE IF SWC IS NOT CONSISTENT
soil_usability <- read.csv("~/data/fLUE/soilm_data_usability_fluxnet2015.csv") # should work both on Euler and local
...
# extract best SM treshold from beni's NP2018 work
file_name = sprintf("~/data/fLUE/profile/profile_light_lue_obs_evi_nn_%s.Rdata", sitename)
load(file_name)

if(settings$varnams_soilm == "SWC_F_MDS_1") {
  settings$threshold = profile_nn_light[[sitename]][["soilm_obs"]][["nnet"]][["best_soilm_trh"]][1]
} else if(settings$varnams_soilm == "wcont_s11") {
  settings$threshold = profile_nn_light[[sitename]][["soilm_splash220"]][["nnet"]][["best_soilm_trh"]][1]
  # simulation s11 has a water holding capacity of 220 mm
}
...
runs <- tuning_run("./R2/keras_grid_search.R",
                   flags = list(
                     nodes1 = c(8, 16, 32, 64),
                     nodes2 = c(8, 16, 32, 64),
                     nodes3 = c(8, 16, 32, 64),
                     ...
                     optimizer = c("adam"),  #leave  always adam (not significant difference with rmsprop)
                     activation = c("relu"), # linear removed because it is never in the best runs
                     batch_size = c(16, 32, 64), # removed 128, 256 (median of runs: 64)
                     epochs = c(10, 20, 30), # removed 50 and 100
                     learning_rate = c(0.01) #, 0.001, 0.0001) take only 0.01 cause it's always the best in previous runs (tried with different sites)
                   ),
                   sample = 0.05, #percentage of the total models to assess: 0.05; way faster: 0.00005
                   runs_dir = runs_path #"./runs" #runs_path # /cluster/scratch/fgiardina don't save output in /runs (otherwise will clog Euler)
)
...
best_run = runs %>% 
  ...
  mutate(Tot_param = sub(
    ".*Trainable params: (.*)\nNon-trainable params: 0\n________________________________________________________________________________\n\n", "\\1",
    model)) %>% # isolate the number of parameters from printed output
  mutate(Tot_param = as.numeric(gsub(",", "", Tot_param))) %>% # correct for commas (if num of parameters > 1000, in the output will be printed as 1,000)
  arrange(Tot_param) %>% 
  slice(1) # among the best 5, take the least complex model
...
# Repeat algorithm with optimal SM threshold and tuned HP
df_train <- prepare_trainingdata_fvar(ddf, settings) 
...
# dunno why but out$df_cv ha solo NA per nn_pot in dry days --> poco male, metto df_all tanto questo R2
# serve solo a provare che non c'è corrispondenza tra nn_pot e obs in dry days 
...
# imp: filter here only (otherwise remove relevant data to train model)
