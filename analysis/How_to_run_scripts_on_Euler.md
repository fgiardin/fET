...
1. With your terminal, connect to the cluster and navigate to the project folder. Then load modules:
...
4. Run the machine learning model. We use a different approach from above: only 13 percent of the sites are run at the same time. A higher percentage will generate too many logs (from the DNN algorithm) at the same time, that will clog the directory where logs are saved on Euler (there is a limited amount of files that each directory on Euler can contain).
